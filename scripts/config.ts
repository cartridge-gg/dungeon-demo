...
//
// Settlement is REAL Starknet Sepolia, so the operator account, the Sepolia RPC, and
// the external USDC address all come from the environment (see .env.example). The
// appchain is external (cartridge-appchain); deploy.ts reads its RPC + account from
// deployments.json, not here. Bun loads `.env` automatically; `up.sh` also exports
